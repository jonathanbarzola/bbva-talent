...
export default function ConstellationView({ graphData, employee, onBack, onExploreEmployee }: ConstellationViewProps) {
  ...
  return (
    <div
      ...
      style={{ background: "#050a14", opacity: visible ? 1 : 0, transition: "opacity 0.35s ease" }}
    >
      {/* ── Top bar ── */}
      <header
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <span className="font-bold text-sm" style={{ color: "#e8eeff" }}>{employee.nombre}</span>
              <span className="font-mono text-[10px] ml-2" style={{ color: "#3d4f6e" }}>{employee.rol}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="font-mono text-[10px] px-2 py-1 rounded-lg" style={{ background: "rgba(133,200,255,0.06)", color: "#3d4f6e" }}>
            Constelación 360°
          </div>
        </div>
      </header>
      ...
      <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
        ...
        <aside
          ...
          style={{ borderRight: "1px solid rgba(133,200,255,0.08)", background: "rgba(5,10,20,0.6)", backdropFilter: "blur(10px)" }}
        >
          {/* Employee card */}
          ...
          {/* Bio */}
          <div className="p-5 border-b" style={{ borderColor: "rgba(133,200,255,0.08)" }}>
            <p className="font-mono text-xs uppercase tracking-widest mb-2.5" style={{ color: "#3d4f6e" }}>Bio</p>
            <p className="text-sm leading-relaxed" style={{ color: "#4d6080" }}>{employee.bio}</p>
          </div>

          {/* Trust Score full */}
          {employee.trust_score && (
            <div className="p-5 border-b" style={{ borderColor: "rgba(133,200,255,0.08)" }}>
              <p className="font-mono text-xs uppercase tracking-widest mb-3" style={{ color: "#3d4f6e" }}>Trust Score</p>
          ...
          {employee.b_tokens && (
            <div className="p-5 border-b" style={{ borderColor: "rgba(133,200,255,0.08)" }}>
              <p className="font-mono text-xs uppercase tracking-widest mb-3" style={{ color: "#3d4f6e" }}>B-Tokens</p>
          ...
          {employee.edi && (
            <div className="p-5 border-b" style={{ borderColor: "rgba(133,200,255,0.08)" }}>
              <p className="font-mono text-xs uppercase tracking-widest mb-3" style={{ color: "#3d4f6e" }}>EDI {employee.edi.año}</p>
              ...
              {employee.colaboradores.map((colab, i) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-mono text-sm truncate font-medium" style={{ color: "#8099b8" }}>{colab.nombre}</p>
                    <p className="font-mono text-xs truncate" style={{ color: "#3d4f6e" }}>{colab.rol}</p>
                  </div>
