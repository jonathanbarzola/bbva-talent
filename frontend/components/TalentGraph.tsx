...
interface TalentGraphProps {
  ...
  height?: number;
  /** When true, fills the parent container's full height instead of a fixed px value */
  fullscreen?: boolean;
}
...
export default function TalentGraph({ data, onNodeClick, height = 560, fullscreen = false }: TalentGraphProps) {
  ...
  // ── Configure d3 forces: strong repulsion + generous link distance ────────
  useEffect(() => {
    const t = setTimeout(() => {
      if (!graphRef.current) return;
      try {
        // Strong negative charge = nodes push away from each other
        graphRef.current.d3Force("charge")?.strength(-600);
        // Longer link distance = more space between connected nodes
  ...
  // ── Zoom to fit after simulation cools ───────────────────────────────────
  useEffect(() => {
    const t = setTimeout(() => graphRef.current?.zoomToFit(600, 100), 1200);
    return () => clearTimeout(t);
  }, [data]);

  // ── Paint node: multi-layer glow + label ─────────────────────────────────
  const paintNode = useCallback(
    (node: GraphNode, ctx: CanvasRenderingContext2D, globalScale: number) => {
      ...
      if (isCentral) {
        // Radial fill for the central node
      ...
      // Label text — always white on dark canvas
  ...
  const paintLink = useCallback((link: GraphLink, ctx: CanvasRenderingContext2D) => {
    ...
    const weight = typeof link.properties?.weight === "number"
      ? link.properties.weight
      : 0.5;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          backgroundImage: "radial-gradient(circle, rgba(133,200,255,0.06) 1px, transparent 1px)",
      ...
      >
        <p className="font-mono text-[9px] uppercase tracking-widest mb-0.5" style={{ color: BBVA.grey4 }}>
          Leyenda
        </p>
        {Object.entries(LEGEND_LABELS).map(([type, label]) => (
          <div key={type} className="flex items-center gap-2">
            <div
              ...
              style={{
                width: NODE_RADIUS[type],
                height: NODE_RADIUS[type],
                backgroundColor: NODE_COLORS[type],
                boxShadow: `0 0 6px ${NODE_COLORS[type]}88`,
              }}
            />
            <span className="text-[10px] font-mono" style={{ color: "#6b7fa3" }}>
              {label}
            </span>
          </div>
        ))}
      ...
      {hoveredNode && (
        <div
          className="absolute top-3 right-3 z-10 max-w-[200px] rounded-xl p-3 animate-fade-in"
          style={{
            background: "rgba(5,10,20,0.9)",
            border: `1px solid ${NODE_COLORS[hoveredNode.type]}44`,
            backdropFilter: "blur(8px)",
            boxShadow: `0 0 20px ${NODE_COLORS[hoveredNode.type]}22`,
          }}
        >
          <p
            className="font-bold text-xs mb-2 leading-tight"
          ...
          {Object.entries(hoveredNode.properties).slice(0, 3).map(([k, v]) => (
            <div key={k} className="flex justify-between gap-2 text-[10px] font-mono">
              ...
              <span style={{ color: "#6b7fa3" }} className="truncate max-w-[100px]">
                {String(v)}
              </span>
            </div>
          ))}
